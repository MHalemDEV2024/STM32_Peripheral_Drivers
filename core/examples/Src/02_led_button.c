 ...
 *
 * 
 *  Description: This program toggles two LEDs on GPIOG pins 13 and 14 based on the state of a board button connected to GPIOA pin 0. 
...
#define GREEN_LED_TOGGLE_DELAY  1  // Delay in seconds
#define RED_LED_TOGGLE_DELAY    2  // Delay in seconds
...
void toggle_leds(void) {
    ...
    if (GPIO_ReadFromInputPin(GPIOA, BUTTON_PIN)) {
        ...
        delay(GREEN_LED_TOGGLE_DELAY);  // Delay for green LED
    } else {
        ...
        delay(RED_LED_TOGGLE_DELAY);  // Delay for red LED
    }
}
